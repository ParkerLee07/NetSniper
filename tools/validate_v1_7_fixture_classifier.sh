...
CLASSIFIER="$ROOT_DIR/tools/classify_v1_7_fixture.py"
...
[[ -x "$CLASSIFIER" ]] || fail "Missing executable classifier: tools/classify_v1_7_fixture.py"
...
for fixture in "$FIXTURE_DIR"/*.json; do
  ...
  output_file="$tmp_dir/${fixture_name%.json}.result.json"
  ...
  jq empty "$output_file" || fail "Classifier produced invalid JSON for $fixture_name"
...
pass "NetSniper v1.7 fixture classifier matched expected outputs"
pass "Validated fixtures: $fixture_count"
