...
if grep -RIn -- '-sU' "$latest_run" >/tmp/netsniper-os-evidence-udp-check.out 2>/dev/null; then
    cat /tmp/netsniper-os-evidence-udp-check.out >&2
    fail "OS evidence stage should not produce UDP-lite evidence"
fi
